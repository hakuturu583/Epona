...
class TrainDataset(Dataset):
        ...
        for video_keys in nuscenes_keys:
            ...
            for token_path_pose in token_path_poses:
                tmp_token_path.append(
                    os.path.join(nuscenes_path, token_path_pose["data_path"])
                )
    ...
    def get_token_feature(self, index):
        ...
        for i in range(self.condition_frames + 1):
            try:
                im = cv2.cvtColor(
                    cv2.imread(token_paths[start + i * self.downsample]),
                    cv2.COLOR_BGR2RGB,
                )
            except:
            ...
            rotation = quaternion_to_rotation_matrix(
                poses[start + i * self.downsample]["rotation"]
            )
            ...
            for token_path_pose in token_path_poses:
                tmp_token_path.append(
                    os.path.join(
                        ...
                        "token",
                        os.path.splitext(
                            os.path.basename(token_path_pose["data_path"])
                        )[0]
                        + ".npy",
                    )
                )
...
class TrainImgDataset(Dataset):
    def __init__(
        ...
        for video_keys in nuscenes_keys:
            ...
            for img_path_pose in img_path_poses:
                tmp_img_path.append(
                    os.path.join(nuscenes_path, img_path_pose["data_path"])
                )
    ...
    def getimg(self, index):
        ...
        start = random.randint(
            0, clip_length - (self.condition_frames + 1) * self.downsample
        )
        ims = []
        poses_new = []
        for i in range(self.condition_frames + 1):
            im = cv2.cvtColor(
                cv2.imread(img_paths[start + i * self.downsample]), cv2.COLOR_BGR2RGB
            )
            h, w, _ = im.shape
            if 2 * h < w:
                w_1 = round(w / h * self.h)
...
class ValImgDataset(Dataset):
    ...
    def getimg(self, index):
        ...
        for i in range(self.condition_frames + 1):
            im = cv2.cvtColor(
                cv2.imread(img_paths[start_index + i * self.downsample]),
                cv2.COLOR_BGR2RGB,
            )
            h, w, _ = im.shape
            if 2 * h < w:
...
class TestImgDataset(Dataset):
    ...
    def getimg(self, index):
        ...
        for i in range(clip_length):
            im = cv2.cvtColor(
                cv2.imread(img_paths[i * self.downsample]), cv2.COLOR_BGR2RGB
            )
            h, w, _ = im.shape
            if 2 * h < w:
                w_1 = round(w / h * self.h)
