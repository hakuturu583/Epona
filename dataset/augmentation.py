...
def random_crop_arr2(
    pil_image, image_width, image_height, min_crop_frac=0.8, max_crop_frac=1.0
):
    ...
    # Thus, we do it by hand to improve downsample quality.
    while (
        pil_image.size[0] >= 2 * smaller_dim_width
        and pil_image.size[1] >= 2 * smaller_dim_height
    ):  # 540 1040/822
        pil_image = pil_image.resize(
            tuple(x // 2 for x in pil_image.size), resample=Image.BOX
        )
...
def random_crop_arr3(
    ...
    croped_pil = Image.fromarray(
        arr[crop_y : crop_y + new_height, crop_x : crop_x + new_width]
    )
...
def np_resize_arr(np_image, image_width, image_height):
    return (
        np.array(
            Image.fromarray(np_image).resize((image_width, image_height), Image.BICUBIC)
        )
        / 255.0
        * 2
        - 1
