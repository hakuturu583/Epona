...
def seek_by_timestamp(
    ...
    assert isinstance(
        query_time, float
    ), f"query_time must be float, not {type(query_time)}"
    assert isinstance(
        t_max_diff, float
    ), f"t_max_diff must be float, not {type(t_max_diff)}"
    ...
    left_time_diff = (
        query_time - timestamps[left_index] if left_index >= 0 else float("inf")
    )
    right_time_diff = (
        timestamps[right_index] - query_time
        if right_index < timestamps.shape[0]
        else float("inf")
    )
    time_diff = min(left_time_diff, right_time_diff)[0]
    if time_diff > t_max_diff:
        raise RuntimeError(
            f"time_diff = {time_diff} is greater than t_max_diff {t_max_diff}"
        )
...
def __absolute2relative(absolute_transform):
    ...
    for idx in range(num_frames - 1):
        ...
        relative_transform_to_pre = (
            invT(absolute_transform[idx]) @ absolute_transform[idx + 1]
        )
...
def load_wheel_pose(wheel_pose_path, sampled_ts=None):
    ...
    for ts in sampled_ts:
        vcs_pose = seek_by_timestamp(
            ts, timestamps, absolute_transform, t_max_diff=0.121
        ).astype("float32")
...
def sample_timestamps(
    input_ts, random_start=False, seq_length=20, frames_downsample_ratio=2
):
