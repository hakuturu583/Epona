...
def add_arguments():
    ...
    parser.add_argument(
        "--resume_step", default=0, type=int, help="continue to train, step"
    )
    parser.add_argument(
        "--load_stt_path", default=None, type=str, help="pretrained path"
    )
    ...
    parser.add_argument(
        "--load_from_deepspeed", default=None, type=str, help="pretrained path"
    )
...
def init_logs(global_rank, args):
    ...
    if global_rank == 0:
        ...
        setup_logger(
            "base", log_path, "train", level=logging.INFO, screen=True, to_file=True
        )
...
def train(local_rank, args):
    ...
    stt_params, dit_params, traj_params = (
        count_parameters(model.model),
        count_parameters(model.dit),
        count_parameters(model.traj_dit),
    )
    print(
        f"Total Parameters: {format_number(stt_params + dit_params)}, SST Parameters: {format_number(stt_params)}, DiT Parameters: {format_number(dit_params)}, TrajDiT Parameters: {format_number(traj_params)}"
    )
    ...
    if args.overfit:
        train_dataset = Subset(
            train_dataset, list(range(4096 - 100, 4096 + 100)) + list(range(0, 200))
        )
    ...
    train_data = DataLoader(
        train_dataset, num_workers=32, batch_sampler=mix_data_sampler
    )
    ...
    while step < args.iter:
        ...
        for i, (img, rot_matrix) in enumerate(train_data):
            ...
            model.train()
            img = img.cuda()
            latents = tokenizer.encode_to_z(
                img[:, : -args.traj_len + args.forward_iter]
            )
            ...
            for j in range(fw_iter):
                ...
                rot_matrix_cond = rot_matrix[
                    :,
                    j * args.block_size : j * args.block_size
                    + args.condition_frames
                    + args.traj_len,
                    ...,
                ]
                ...
                if j < fw_iter - 1:
                    if args.return_predict:
                        ...
                        predict_traj, predict_latents = model(
                            latents_cond, rot_matrix_cond, latents_gt, sample_last=False
                        )
                        model.train()
                    latents_cond = rearrange(
                        predict_latents,
                        "(b t) l c -> b t l c",
                        b=args.batch_size,
                        t=args.condition_frames // args.block_size,
                    )
                    rel_traj_cond = rearrange(
                        predict_traj,
                        "(b t) l c -> b t l c",
                        b=args.batch_size,
                        t=args.condition_frames // args.block_size,
                    )[:, :, 0, :]
                    rel_pose_cond, rel_yaw_cond = (
                        rel_traj_cond[..., 0:2],
                        rel_traj_cond[..., 2:3],
                    )
            step += 1
            ...
            if step % 100 == 1 and rank == 0:
                ...
                writer.add_scalar(
                    "loss/loss_all", loss_final["loss_all"].to(torch.float32), step
                )
                writer.add_scalar(
                    "loss/loss_diff", loss_final["loss_diff"].to(torch.float32), step
                )
                writer.add_scalar(
                    "loss/loss_yaw_pose",
                    loss_final["loss_yaw_pose"].to(torch.float32),
                    step,
                )
                writer.flush()
            if rank == 0:
                logger.info(
                    "step:{} time:{:.2f}+{:.2f} lr:{:.4e} loss_avg:{:.4e} diff_loss:{:.4e} pose_loss:{:.4e} ".format(
                        step,
                        data_time_interval,
                        train_time_interval,
                        optimizer.param_groups[0]["lr"],
                        loss_final["loss_all"].to(torch.float32),
                        loss_final["loss_diff"].to(torch.float32),
                        loss_final["loss_yaw_pose"].to(torch.float32),
                    )
                )
            if step % args.eval_steps == 0:  # or (step == 1):
                ...
                torch.cuda.synchronize()
                save_ckpt_deepspeed(
                    args, save_model_path, model, optimizer, lr_schedule, step
                )
                dist.barrier()
                if rank == 0:
                    save_ckpt(
