...
def _ensure_nuscenes_meta(repo_root: str):
    ...
    with torch.no_grad():
        for i, (img, rot_matrix) in tqdm(enumerate(val_data)):
            video_save_path = os.path.join(
                args.save_video_path, args.exp_name, "sliding_" + str(i + args.start_id)
            )
            ...
            for j in range(condition_frames):
                img_pred = tokenizer.z_to_image(
                    rearrange(
                        ...
                        "b (h w) c -> b h w c",
                        h=args.image_size[0]
                        // (args.downsample_size * args.patch_size),
                        w=args.image_size[1]
                        // (args.downsample_size * args.patch_size),
                    )
                )
                img_pred = (img_pred[0].permute(1, 2, 0).cpu().numpy() * 255).astype(
                    "uint8"
                )[:, :, ::-1]
            ...
            for t1 in range(0, args.test_video_frames, args.temporal_patch_size):
                ...
                for t2 in range(args.temporal_patch_size):
                    ...
                    predict_latents = rearrange(
                        predict_latents, "(b f) h w c -> b f (h w) c", f=1
                    )
                    start_latents = torch.cat(
                        (start_latents[:, 1:condition_frames, ...], predict_latents),
                        dim=1,
                    )

                predict_latents_patch = torch.cat(predict_latents_patch, dim=0)
                img_preds = tokenizer.z_to_image(
                    predict_latents_patch, is_video=is_video
                ).cpu()
                for t2, img_pred in enumerate(img_preds):
                    img_pred_np = (img_pred.permute(1, 2, 0).numpy() * 255).astype(
                        "uint8"
                    )[:, :, ::-1]
                    cv2.imwrite(
                        os.path.join(
                            video_save_path, "%d.png" % (t1 + t2 + condition_frames)
                        ),
                        img_pred_np,
                    )
...
def main(args):
    ...
    meta_dir = _ensure_nuscenes_meta(repo_root)
    nuscenes_json_path = os.path.join(meta_dir, "nuscenes_val.json")
    ...
    test_dataset = Subset(
        test_dataset, list(range(start_id, end_id))
    )  # +list(range(100-10, 100, 2))+list(range(1000-10, 1000+5, 2)))
