    ...
    with torch.no_grad():
        for i, (img, rot_matrix) in tqdm(enumerate(val_data)):
            video_save_path = os.path.join(
                args.save_video_path, args.exp_name, "sliding_" + str(i)
            )
            ...
            for t1 in range(
                len(yaw_list)
            ):  # range(min(pose.shape[1]-(condition_frames+1), 60)):
                ...
                pose_new = (
                    torch.from_numpy(pose_new).cuda().unsqueeze(dim=0).unsqueeze(dim=1)
                )
                ...
                img_pred_np = (img_pred[0].permute(1, 2, 0).cpu().numpy() * 255).astype(
                    "uint8"
                )[:, :, ::-1]
                ...
                cv2.imwrite(
                    os.path.join(video_save_path, "%d.png" % (t1 + condition_frames)),
                    img_pred_np,
                )
