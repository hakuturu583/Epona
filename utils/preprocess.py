...
def get_rel_poses_from_abs(pose_array):
    ...
    relative_poses[:, 0] = torch.tensor(
        [0, 0, 0], dtype=pose_array.dtype, device=pose_array.device
    )
    ...
    rel_poses[:, :, 2] = torch.atan2(
        torch.sin(rel_poses[:, :, 2]), torch.cos(rel_poses[:, :, 2])
    )
...
def get_rel_traj_from_abs(poses, condition_frames, traj_len):
    ...
    targets = torch.stack(
        [poses[:, i : i + N, :] for i in range(1, F + 1)], dim=1
    )  # [B, F, N, 3]
...
def get_rel_traj_from_abs_np(poses, condition_frames, traj_len):
    ...
    origins = poses[:, :F, None, :]  # 选取所有起始点，形状 [B, F, 1, 3]
    targets = np.stack(
        [poses[:, i : i + N, :] for i in range(F)], axis=1
    )  # [B, F, N, 3]
...
def get_rel_pose_tr(poses):
    ...
    # 计算相对偏航角（yaw）
    rel_yaws = radians_to_degrees(
        extract_yaw_from_rotation_matrices(rot_matrices[..., :3, :3])
    ).unsqueeze(-1)
...
def get_rel_pose(poses):
    ...
    rel_yaws = radians_to_degrees(
        extract_yaw_from_rotation_matrices(rel_pose_rot[..., :3, :3])
    ).unsqueeze(-1)
    return rel_poses, rel_yaws
...
def get_rel_traj_tr(poses, condition_frames, traj_len):
    ...
    # 选取每一帧后 N 帧的相对位姿
    indices = torch.arange(F).unsqueeze(1) + torch.arange(1, N + 1).unsqueeze(
        0
    )  # [F, N]
    indices = torch.clamp(indices, max=F + N - 2)

    rot_matrices = rot_matrices[
        :, torch.arange(F).unsqueeze(1), indices
    ]  # [B, F, N, 4, 4]
