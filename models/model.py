...
class TrainTransformersDiT(nn.Module):
    ...
    def model_forward(
            ...
            with torch.cuda.amp.autocast(enabled=False):
                rel_pose_gt, rel_yaw_gt = get_rel_pose(
                    rot_matrix[
                        ...
                        * self.args.block_size : (self.condition_frames + 1)
        ...
        yaw_indices_total = yaws_to_indices(
            rel_yaw_total, self.yaw_vocab_size
        )  # (b, t+n, 1)
        logits = self.model(
            feature_total,
            pose_indices_total,
        ...
        with torch.cuda.amp.autocast(enabled=False):
            traj_poses, traj_yaws = get_rel_traj(
                rot_matrix, self.condition_frames, self.traj_len
            )
    ...
    def step_train(
        ...
        if pro < self.args.mask_data:
            mask = torch.bernoulli(
                random.uniform(0.7, 1) * torch.ones_like(latents_total)
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.training:
            ...
            return self.step_eval(
                latents, rot_matrix, sample_last=sample_last, **kwargs
            )
    ...
    def step_eval(
        ...
        start_time = time.time()
        self.traj_dit.eval()
        noise_traj = torch.randn(bsz, self.traj_len, self.traj_token_size).to(
            stt_features
        )
        ...
        if traj_only:
            ...
            if self_pred_traj:
                ...
                predict_yaw = yaws_to_indices(
                    predict_yaw, self.yaw_vocab_size
                )  # (b, 1, 1)
                pose_emb = self.model.get_pose_emb(predict_pose, predict_yaw)

            start_time = time.time()
            self.dit.eval()
            noise = torch.randn(bsz, self.img_token_size, self.vae_emb_dim).to(
                stt_features
            )
            timesteps = get_schedule(
                int(self.args.num_sampling_steps), self.img_token_size
            )
            predict_latents = self.dit.sample(
                noise, img_ids, stt_features, cond_ids, pose_emb, timesteps
            )
            predict_latents = rearrange(
                predict_latents, "b (h w) c -> b h w c", h=self.h, w=self.w
            )
            interval = time.time() - start_time
