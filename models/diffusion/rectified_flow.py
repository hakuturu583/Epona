...
class RectifiedFlow:
    ...
    def training_losses(
        ...
        if dynamics_enhance:
            ...
            if vis_root_path:
                ...
                aux_loss_ = F.normalize(
                    aux_loss_clone[:, :4].reshape(bs, -1), p=1
                ).reshape(bs, 4, aux_loss_c, aux_loss_h, aux_loss_w)
                ...
                standard_loss_ = (
                    (standard_loss * aux_loss_[0]).sum(dim=1).detach().cpu().numpy()
                )
                ...
                for map_id in range(4):
                    ...
                    file_name = os.path.join(
                        vis_root_path, f"weight_viridis_first_{map_id}.png"
                    )
                    plt.axis("off")
                    ...
                    file_name = os.path.join(
                        vis_root_path, f"weight_hot_first_{map_id}.png"
                    )
                    plt.axis("off")
                    plt.gca().set_axis_off()
                    plt.subplots_adjust(
                    ...
                    fig, ax = plt.subplots()
                    ax.matshow(
                        standard_loss[map_id], cmap="viridis", interpolation="lanczos"
                    )
                    file_name = os.path.join(
                        vis_root_path, f"diffloss_before_viridis_first_{map_id}.png"
                    )
                    plt.axis("off")
                    plt.gca().set_axis_off()
                    plt.subplots_adjust(
                    ...
                    fig, ax = plt.subplots()
                    ax.matshow(
                        standard_loss[map_id], cmap="hot", interpolation="lanczos"
                    )
                    file_name = os.path.join(
                        vis_root_path, f"diffloss_before_hot_first_{map_id}.png"
                    )
                    plt.axis("off")
                    plt.gca().set_axis_off()
                    plt.subplots_adjust(
                    ...
                    fig, ax = plt.subplots()
                    ax.matshow(
                        standard_loss_[map_id], cmap="viridis", interpolation="lanczos"
                    )
                    file_name = os.path.join(
                        vis_root_path, f"diffloss_after_viridis_first_{map_id}.png"
                    )
                    plt.axis("off")
                    plt.gca().set_axis_off()
                    plt.subplots_adjust(
                    ...
                    fig, ax = plt.subplots()
                    ax.matshow(
                        standard_loss_[map_id], cmap="hot", interpolation="lanczos"
                    )
                    file_name = os.path.join(
                        vis_root_path, f"diffloss_after_hot_first_{map_id}.png"
                    )
                    plt.axis("off")
                    plt.gca().set_axis_off()
                    plt.subplots_adjust(
            ...
            aux_w = 1 + torch.cat(
                (torch.zeros(aux_w.shape[0], 1, *aux_w.shape[2:]).to(aux_w), aux_w),
                dim=1,
            )
    ...
    def sample(self, model, shape, noise=None, model_kwargs=None):
