...
class SpatialTemporalTransformer(nn.Module):
    ...
    def forward(
        self, feature_total, pose_indices_total, yaw_indices_total, drop_feature=0
    ):
        """
        Args:
        ...
        target_yaw_token_embeddings = yaw_token_embeddings[
            :, self.temporal_block :, ...
        ].reshape(B * F, self.temporal_block * self.C)
        pose_emb = torch.cat(
            [
    ...
    def evaluate(self, feature, pose_total, yaw_total, sample_last=True):
        ...
        if sample_last:
            ...
            target_yaw_token_embeddings = yaw_token_embeddings[:, -1, ...].reshape(
                B, self.C
            )
        else:
            target_pose_x_token_embeddings = pose_x_token_embeddings[
    ...
    def get_pose_emb(self, pose, yaw):
        ...
        pose_emb = torch.cat(
            [pose_x_token_embeddings, pose_y_token_embeddings, yaw_token_embeddings],
            dim=-1,
        ).reshape(-1, self.C * 3)
        return pose_emb
